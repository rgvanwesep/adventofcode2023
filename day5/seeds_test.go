...
func TestMinLocationRangeAtlas(t *testing.T) {
	cases := []struct {
		inputs   []string
		expected int
	}{
	...
	for _, c := range cases {
		actual := MinLocationFromRangeAtlas(c.inputs)
		if actual != c.expected {
			t.Errorf("MinLocationFromRangeAtlas(%v) == %v, want %v", c.inputs, actual, c.expected)
		}
	}
}